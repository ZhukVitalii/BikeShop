...
import beetle.entity.handlebars.Winding;
...
public interface WindingRepository extends JpaRepository<Winding, Long> {
    @Query("SELECT c FROM Winding c WHERE c.manufacturer = :manufacturer")
    List<Winding> findByHandlebarMakers(@Param("manufacturer") Manufacturer manufacturer, Pageable pageable);

    @Query("SELECT COUNT(c) FROM Winding c WHERE c.manufacturer = :manufacturer")
