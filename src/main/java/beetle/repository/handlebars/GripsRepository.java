...
import beetle.entity.handlebars.Grips;
...
public interface GripsRepository extends JpaRepository<Grips, Long> {
    @Query("SELECT c FROM Grips c WHERE c.manufacturer = :manufacturer")
    List<Grips> findByHandlebarMakers(@Param("manufacturer") Manufacturer manufacturer, Pageable pageable);

    @Query("SELECT COUNT(c) FROM Grips c WHERE c.manufacturer = :manufacturer")
