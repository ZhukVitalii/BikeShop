...
public interface FrontHubRepository extends JpaRepository<FrontHub, Long> {
    ...
    @Query("SELECT c FROM FrontHub c WHERE c.manufacturer = :manufacturer")
    List<FrontHub> findByWheelMakers(@Param("manufacturer") Manufacturer manufacturer, Pageable pageable);

    @Query("SELECT COUNT(c) FROM FrontHub c WHERE c.manufacturer = :manufacturer")
