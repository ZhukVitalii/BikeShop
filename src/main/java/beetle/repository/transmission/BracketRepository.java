...
public interface BracketRepository extends JpaRepository<Bracket, Long> {
    ...
    @Query("SELECT c FROM Bracket c WHERE c.manufacturer = :manufacturer")
    List<Bracket> findByTransmissionMakers(@Param("manufacturer") Manufacturer manufacturer, Pageable pageable);

    @Query("SELECT COUNT(c) FROM Bracket c WHERE c.manufacturer = :manufacturer")
