...
public interface ForkRepository extends JpaRepository<Fork, Long> {
    @Query("SELECT c FROM Fork c WHERE c.manufacturer = :manufacturer")
    List<Fork> findByForkMakers(@Param("manufacturer") Manufacturer manufacturer, Pageable pageable);
    ...
    @Query("SELECT COUNT(c) FROM Fork c WHERE c.manufacturer = :manufacturer")
    long countByForkMaker(@Param("manufacturer") Manufacturer manufacturer);
