...
<html>
<head>
    <title>New backDerailleur</title>
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.1/css/bootstrap.min.css">
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.1/js/bootstrap.min.js"></script>
</head>
<body>
<div class="container">
    <form role="form" class="form-horizontal" action="/backDerailleur/add" method="post">
        ...
        <br>
        <select class="btn btn-primary btn-select btn-select-light" name="backSprocketMax">
            <option value="-1">Найбільша зірка</option>
            <c:forEach items="${backSprocketMax}" var="backSprocketMax">
                <option value="${backSprocketMax.id}">${backSprocketMax.number}</option>
            </c:forEach>
        </select>
        <br>
        <select class="btn btn-primary btn-select btn-select-light" name="backSprocketMin">
            <option value="-1">Найменша зірка</option>
            <c:forEach items="${backSprocketMin}" var="backSprocketMin">
                <option value="${backSprocketMin.id}">${backSprocketMin.number}</option>
            </c:forEach>
        </select>
        <br>
        <select class="btn btn-primary btn-select btn-select-light" name="pawLength">
            <option value="-1">Довжина лапки</option>
            <c:forEach items="${pawLength}" var="pawLength">
                <option value="${pawLength.id}">${pawLength.length}</option>
            </c:forEach>
        </select>
        <br>
