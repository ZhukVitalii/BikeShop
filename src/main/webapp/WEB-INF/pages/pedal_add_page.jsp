...
<body>
<div class="container">
    <form role="form" class="form-horizontal" action="/pedal/add" method="post">
        <h3>Додати педалі</h3>
        <select class="btn btn-primary btn-select btn-select-light" name="transmissionMaker">
            <option value="-1">Виробник</option>
            <c:forEach items="${transmissionMaker}" var="transmissionMaker">
                <option value="${transmissionMaker.id}">${ransmissionMaker.name}</option>
            </c:forEach>
        </select>
        <br>
        <input class="form-control " type="text" name="name" placeholder="Назва">
        <br>
        <select class="btn btn-primary btn-select btn-select-light" name="pedalType">
            <option value="-1">Тип педалей</option>
            <c:forEach items="${pedalType}" var="pedalType">
        ...
        <br>
        <input class="form-control " type="text" name="sizes" placeholder="Розміри">
        <br>
